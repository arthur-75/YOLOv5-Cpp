...
std::string main_path= "/Users/arthur/Documents/work/AL_work/ACAP/AL_CV/AL_CV/";
    ...
    for (int i = 0; i < rows; ++i) {
        ...
        if (confidence >= CONFIDENCE_THRESHOLD) {
            ...
            if (max_class_score > SCORE_THRESHOLD) {
                //std::cout <<className.size()<< std::endl;
...
int main(int argc, char **argv)
{
    ...
    cv::Mat frame;
    const std::string videoStreamAddress = "rtsp://root:root@169.254.210.203/axis-media/media.amp"; // this for the camera location
    std::string main_path= "/Users/arthur/Documents/work/AL_work/ACAP/AL_CV/AL_CV/";// in case if there is a video
    cv::VideoCapture capture(0);//(0//main_path+"sample.mp4"
                             //);capture(videoStreamAddress)
    if (!capture.isOpened())// if there is a video or camera
    {
    ...
    while (true)
    {
        ...
        int long detections = output.size();
